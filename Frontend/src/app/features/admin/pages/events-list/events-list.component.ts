  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <h1 class="text-3xl font-black text-gray-900 uppercase tracking-tight">{{ 'ADMIN.EVENTS_LIST.TITLE' | translate }}</h1>
        <div class="flex gap-3">
             <button (click)="openCategoryModal()" class="px-5 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-2 font-bold uppercase tracking-wider text-sm shadow-sm">
              <i class="fas fa-tags"></i> {{ 'ADMIN.EVENTS_LIST.BTN_CATS' | translate }}
            </button>
            <a routerLink="/admin/events/new" class="px-5 py-2.5 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition-colors flex items-center gap-2 font-bold uppercase tracking-wider text-sm shadow-lg shadow-pink-200">
              <i class="fas fa-plus"></i> {{ 'ADMIN.EVENTS_LIST.BTN_CREATE' | translate }}
            </a>
        </div>
      </div>
      ...
      <div class="flex gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
         <div class="relative flex-1">
            <input type="text" [(ngModel)]="searchQuery" (keyup.enter)="onSearch()" [placeholder]="'ADMIN.EVENTS_LIST.SEARCH' | translate" class="w-full pl-10 pr-4 py-2.5 rounded-lg border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all text-sm font-medium">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-100">
            <thead class="bg-gray-50">
              <tr>
                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ 'ADMIN.EVENTS_LIST.TH_EVENT' | translate }}</th>
                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ 'ADMIN.EVENTS_LIST.TH_DATE' | translate }}</th>
                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ 'ADMIN.EVENTS_LIST.TH_VENUE' | translate }}</th>
                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ 'ADMIN.EVENTS_LIST.TH_STATUS' | translate }}</th>
                <th scope="col" class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">{{ 'ADMIN.EVENTS_LIST.TH_ACTION' | translate }}</th>
              </tr>
            </thead>
              ...
              @for (event of events(); track event.id) {
                <tr class="hover:bg-pink-50/30 transition-colors group">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <a [routerLink]="['/admin/events/edit', event.id]" class="text-pink-600 hover:text-pink-900 mr-4 font-bold uppercase text-xs tracking-wider">{{ 'ADMIN.EVENTS_LIST.BTN_EDIT' | translate }}</a>
                    <button (click)="deleteEvent(event.id)" class="text-gray-400 hover:text-red-600 font-bold uppercase text-xs tracking-wider transition-colors">{{ 'ADMIN.EVENTS_LIST.BTN_DELETE' | translate }}</button>
                  </td>
                </tr>
              }
              @if (events().length === 0) {
                <tr>
                  <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                    {{ 'ADMIN.EVENTS_LIST.EMPTY' | translate }}
                  </td>
                </tr>
              }
            </tbody>
